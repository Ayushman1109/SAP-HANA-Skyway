...
curl.exe -fs --max-time 3 http://localhost:80/gilhari/v1/health/check >nul 2>&1
if not errorlevel 1 (
...
docker run --platform linux/amd64 --hostname hana-poc-service -d --name hana-poc-service -p 80:8081 hana-poc-service:1.0

echo Waiting for Gilhari microservice to start...
echo (This may take up to 3 minutes for cloud or remote databases)
set READY=0
for /L %%i in (1,1,18) do (
    curl.exe -fs http://localhost:80/gilhari/v1/health/check >nul 2>&1
...
if !READY!==1 (
    ...
    echo To check if it started later: curl.exe -s http://localhost:80/gilhari/v1/health/check
    exit /b 1
)
